...
public class Humidifier extends Device{
    public Humidifier(String name, String deviceKind, String model, int state, String room) {
    ...
    public void onoffDevice() {
        switch (state){                 // 디바이스 상태
            case 1:                     // 켜져있을 경우
                ...
                if(!APP_TEST) commandDevice(CMD_ACL_AI);
                break;
            case 2:                     // 꺼져있을 경우
                setState(1);            // 켜짐으로 변경
                if(!APP_TEST) commandDevice(CMD_ACL_ON);
                break;
            case 3:                     // 인공지능 모드일 경우
                setState(2);            // 꺼짐으로 변경
                if(!APP_TEST) commandDevice(CMD_ACL_OFF);
                break;
        }
    }
